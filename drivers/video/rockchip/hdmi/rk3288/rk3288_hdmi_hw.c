...
/*	{74250000,	0,	8,	0,	0,	0,	1,	3,	0,	2,	5,	0,	1}, */
	...
	{297000000,	0,	10,	1,	3,	1,	5,	1,	3,	1,	7,	0,	3},
	{297000000,	0,	12,	2,	3,	1,	1,	2,	0,	1,	7,	0,	3},
	{297000000,	0,	16,	3,	3,	1,	1,	3,	1,	0,	0,	0,	3},
...
static void rk3288_i2cm_write_request(struct hdmi_dev *hdmi_dev,
				u8 offset, u8 data)
{
...
static void rk3288_i2cm_write_data(struct hdmi_dev *hdmi_dev,
				u8 data, u8 offset)
{
	u8 interrupt;
	int trytime = 2;
...
static void rk3288_scdc_set_source_version(struct hdmi_dev *hdmi_dev,
					   u8 version)
{
...
static void rk3288_scdc_enable_read_request(struct hdmi_dev *hdmi_dev,
					    int enable)
{
...
static void rk3288_scdc_enable_polling(struct hdmi_dev *hdmi_dev,
				       int enable)
{
...
{
	rk3288_hdmi_i2cm_reset(hdmi_dev);
	rk3288_hdmi_i2cm_mask_int(hdmi_dev, 1);
	rk3288_hdmi_i2cm_clk_init(hdmi_dev);
	/* set scdc i2c addr */
...
static const struct phy_mpll_config_tab *get_phy_mpll_tab(
		unsigned int pixClock, char pixRepet, char colorDepth)
{
	int i;
					   ...
					   struct hdmi_video *vpara)
{
	...
	pr_info("pixel clk is %u tmds clk is %u last tmds clk %u tmdsclk_ratio_change %d\n",
		mode->pixclock, tmdsclk, hdmi_dev->tmdsclk, hdmi_dev->tmdsclk_ratio_change);
	...
	if (hdmi_drv->edid.scdc_present == 1) {
		if (tmdsclk > 340000000) {/* used for HDMI 2.0 TX */
			...
			rk3288_scdc_set_source_version(hdmi_dev, hdmi_drv->edid.hf_vsdb_version);
	...
	hdmi_writel(hdmi_dev, FC_EXCTRLSPAC, (hdmi_dev->tmdsclk/1000) * 50 / (256 * 512));
	hdmi_msk_reg(hdmi_dev, MC_SWRSTZREQ,
		     m_TMDS_SWRST, v_TMDS_SWRST(0));
...
static int rk3288_hdmi_video_packetizer(struct hdmi_dev *hdmi_dev,
					struct hdmi_video *vpara)
{
...
static int rk3288_hdmi_video_sampler(struct hdmi_dev *hdmi_dev,
				     struct hdmi_video *vpara)
{
	int map_code = 0;
...
static int rk3288_hdmi_video_csc(struct hdmi_dev *hdmi_dev,
				 struct hdmi_video *vpara)
{
...
static int hdmi_dev_read_edid(struct hdmi *hdmi, int block, unsigned char *buff)
{
	...
	rk3288_hdmi_i2cm_mask_int(hdmi_dev, 0);
			...
			struct hdmi_video *vpara)
{
	...
	/* AVI use RDRB mode, auto mode can not pass CTS 7-19 */
	hdmi_msk_reg(hdmi_dev, FC_DATAUTO3, m_AVI_AUTO, v_AVI_AUTO(0));
	hdmi_writel(hdmi_dev, FC_RDRB6, 0x00);
	hdmi_writel(hdmi_dev, FC_RDRB7, 0x10);
				...
				struct hdmi_audio *audio)
{
	...
	/*RDRB mode, auto mode can not pass CTS*/
	hdmi_msk_reg(hdmi_dev, FC_DATAUTO3, m_AAI_AUTO, v_AAI_AUTO(0));
	hdmi_writel(hdmi_dev, FC_RDRB2, 0x00);
	hdmi_writel(hdmi_dev, FC_RDRB3, 0x10);
}
...
static int hdmi_dev_config_audio(struct hdmi *hdmi, struct hdmi_audio *audio)
{
	...
	if (hdmi_dev->audiosrc == INPUT_SPDIF) {
		...
		} else if (HDMI_AUDIO_FS_44100 != audio->rate) {
			pr_info("nlpcm mode\n");
...
static int hdmi_dev_control_output(struct hdmi *hdmi, int enable)
{
	...
	if (enable == HDMI_AV_UNMUTE) {
		...
		if (enable & HDMI_VIDEO_MUTE) {
			...
			vpara.vic = hdmi->vic & HDMI_VIC_MASK;
			vpara.color_output = HDMI_COLOR_RGB_0_255;
			hdmi_dev_config_avi(hdmi_dev, &vpara);
...
static int hdmi_dev_enable(struct hdmi *hdmi)
{
	...
	if (!hdmi_dev->enable) {
		hdmi_dev->enable = 1;
		hdmi_msk_reg(hdmi_dev, PHY_CONF0,
			     m_ENHPD_RXSENSE_SIG, v_ENHPD_RXSENSE_SIG(1));
		hdmi_writel(hdmi_dev, IH_MUTE, 0x0);
	}
...
void rk3288_hdmi_dev_init_ops(struct hdmi_ops *ops)
{
	if (ops) {
...
void rk3288_hdmi_dev_initial(struct hdmi_dev *hdmi_dev)
{
	...
	if (hdmi_dev->hdcp_enable)
		rk3288_hdmi_hdcp_init(hdmi);
}

irqreturn_t rk3288_hdmi_dev_irq(int irq, void *priv)
{
	...
	if (hdcp_int) {
		...
		rk3288_hdmi_hdcp_isr(hdmi_dev, hdcp_int);
//		pr_info("sate is 0x%02x\n", hdmi_readl(hdmi_dev, 0x5002));
	}
	return IRQ_HANDLED;
}
